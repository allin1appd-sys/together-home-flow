...
const Groceries = () => {
  const { groceries, isLoading: gLoading, addGrocery, removeGrocery, decrementGrocery } = useGroceries();
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="inventory" className="space-y-3 mt-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={() => setGrocerySheet(true)} className="ml-auto gap-1"><Plus className="h-4 w-4" /> Add</Button>
          </div>
          {filtered.length === 0 ? (
            <div className="text-center py-12"><p className="text-muted-foreground">Inventory is empty — <button onClick={() => setGrocerySheet(true)} className="text-primary underline">add your first item</button></p></div>
          ) : (
            <AnimatePresence>
              {filtered.map((item) => (
                <motion.div key={item.id} layout initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0, height: 0 }}>
                  <SwipeableGroceryCard item={item} onDecrement={decrementGrocery} onDelete={(id) => setDeleteId(id)} />
                </motion.div>
              ))}
            </AnimatePresence>
          )}
