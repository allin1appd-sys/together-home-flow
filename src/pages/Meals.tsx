...
const SwipeableMealCell = ({ meal, type, onTap, onDelete }: { meal: MealPlan | undefined; type: MealType; onTap: () => void; onDelete: () => void }) => {
  const x = useMotionValue(0);
  const bgOpacity = useTransform(x, [-100, -50, 0], [1, 0.5, 0]);
  if (!meal) return <button onClick={onTap} className="text-left rounded-lg p-2 text-xs transition-colors bg-muted/50 hover:bg-muted"><span className="text-muted-foreground">{mealEmoji[type]} {type}</span><p className="text-muted-foreground mt-0.5">+ Add</p></button>;
  return (
    <div className="relative overflow-hidden rounded-lg">
      <motion.div className="absolute inset-0 flex items-center justify-end pr-3 bg-destructive" style={{ opacity: bgOpacity }}><Trash2 className="h-4 w-4 text-destructive-foreground" /></motion.div>
      <motion.button className="relative text-left rounded-lg p-2 text-xs bg-accent w-full" style={{ x }} drag="x" dragConstraints={{ left: -120, right: 0 }} dragElastic={0.1} onDragEnd={(_: any, info: PanInfo) => { if (info.offset.x < SWIPE_THRESHOLD) onDelete(); }} onClick={onTap}>
...
const Meals = () => {
  ...
  const handleCopyLastWeek = () => { copyLastWeekMeals(weekStartStr); toast.success('Copied meals from last week'); };

  const handleAddRecipe = () => {
    if (!rName.trim()) return;
    addRecipe({ id: Date.now().toString(), name: rName.trim(), instructions: rInstructions, prepTime: parseInt(rPrepTime) || 0, cookTime: 0, servings: 2, tags: rTags.split(',').map(t => t.trim()).filter(Boolean), ingredients: [] });
