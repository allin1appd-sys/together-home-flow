...
const categoryIcon: Record<ReminderCategory, string> = {
  'Bills & Payments': '💳',
  'Health & Medical': '🏥',
  'Home Maintenance': '🔧',
  'Subscriptions': '📺',
...
const SwipeableReminderCard = ({ reminder, onTap, onDelete, onToggle, onSnooze }: SwipeableReminderCardProps) => {
            ...
            <div className="flex items-start gap-3">
              <button
                ...
                className={`mt-0.5 h-5 w-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                  reminder.isChecked ? 'bg-primary border-primary' : isOverdue ? 'border-destructive' : 'border-muted-foreground/40'
                }`}
              >
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  ...
                  {reminder.repeat !== 'none' && (
                    <Badge variant="secondary" className="text-[10px] px-1.5 py-0 gap-0.5">
                      <RotateCcw className="h-2.5 w-2.5" />{reminder.repeat}
                    </Badge>
                  )}
                </div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <button onClick={(e) => e.stopPropagation()} className="p-1.5 rounded-lg hover:bg-muted transition-colors shrink-0">
...
const Reminders = () => {
  const { reminders, addReminder, updateReminder, deleteReminder, toggleReminder, snoozeReminder } = useHomeStore();
  ...
  const filtered = reminders.filter((r) => {
    if (filter === 'active') return !r.isChecked;
    if (filter === 'done') return r.isChecked;
    return true;
  });
  ...
  const openEdit = (r: Reminder) => {
    ...
    setForm({ title: r.title, description: r.description || '', dueDate: r.dueDate, isChecked: r.isChecked, category: r.category, leadDays: r.leadDays, repeat: r.repeat, createdAt: r.createdAt });
    setSheetOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingReminder) {
      updateReminder({ ...editingReminder, ...form });
    } else {
      addReminder({ id: `rem-${Date.now()}`, ...form });
    }
    setSheetOpen(false);
  };
    ...
    <div className="px-4 pt-6 pb-24 space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground">No reminders {filter !== 'all' ? `(${filter})` : ''}</p>
          <p className="text-xs text-muted-foreground mt-1">Tap + to add your first reminder!</p>
        </div>
      ) : (
          ...
          {filtered
            ...
            .map((reminder) => (
              <SwipeableReminderCard
                key={reminder.id}
                reminder={reminder}
                onTap={() => openEdit(reminder)}
                onDelete={() => setDeleteId(reminder.id)}
                onToggle={() => toggleReminder(reminder.id)}
                onSnooze={(until) => snoozeReminder(reminder.id, until)}
              />
            ))}
        </div>
      )}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl max-h-[85vh] overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>{editingReminder ? 'Update reminder details' : 'Never forget again'}</SheetDescription>
          </SheetHeader>

          <div className="space-y-4 mt-4">
            <div className="space-y-1.5">
              <Label>Title</Label>
              <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Pay electricity bill" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" min={0} max={30} value={form.leadDays} onChange={(e) => setForm({ ...form, leadDays: parseInt(e.target.value) || 0 })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>Category</Label>
                <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v as ReminderCategory })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {CATEGORIES.map((c) => (
                      <SelectItem key={c} value={c}>{categoryIcon[c]} {c}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label>Repeat</Label>
                <Select value={form.repeat} onValueChange={(v) => setForm({ ...form, repeat: v as ReminderRepeat })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {REPEATS.map((r) => (
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (deleteId) { deleteReminder(deleteId); setSheetOpen(false); } setDeleteId(null); }}
      />
    </div>
  );
};
