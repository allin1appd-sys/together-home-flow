...
const Budget = () => {
  ...
  const monthlyTransactions = useMemo(() => transactions.filter(t => isWithinInterval(parseISO(t.date), { start: monthStart, end: monthEnd })), [transactions, monthStart.getTime(), monthEnd.getTime()]);
  ...
  const categoryData = useMemo(() => {
    ...
    monthlyTransactions.filter(t => t.type === 'expense').forEach(t => map.set(t.category, (map.get(t.category) || 0) + t.amount));
  ...
  const openEdit = (t: Transaction) => {
    ...
    setForm({ description: t.description, amount: String(t.amount), category: t.category, type: t.type });
    setSheetOpen(true);
  };
    ...
    <PullToRefresh queryKeys={[['transactions', householdId!], ['budget_limits', householdId!]]}>
      <div className="px-4 pt-6 pb-24 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Card><CardContent className="p-3 text-center"><TrendingUp className="h-4 w-4 text-green-600 dark:text-green-400 mx-auto mb-1" /><p className="text-xs text-muted-foreground">Income</p><p className="font-bold text-sm text-green-600 dark:text-green-400">${totalIncome.toFixed(0)}</p></CardContent></Card>
          <Card><CardContent className="p-3 text-center"><TrendingDown className="h-4 w-4 text-red-600 dark:text-red-400 mx-auto mb-1" /><p className="text-xs text-muted-foreground">Expenses</p><p className="font-bold text-sm text-red-600 dark:text-red-400">${totalExpenses.toFixed(0)}</p></CardContent></Card>
          <Card><CardContent className="p-3 text-center"><DollarSign className="h-4 w-4 text-primary mx-auto mb-1" /><p className="text-xs text-muted-foreground">Balance</p><p className={`font-bold text-sm ${balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>${balance.toFixed(0)}</p></CardContent></Card>
        </div>

        <Tabs defaultValue="transactions">
          <TabsList className="grid w-full grid-cols-2"><TabsTrigger value="transactions">Transactions</TabsTrigger><TabsTrigger value="breakdown">Breakdown</TabsTrigger></TabsList>
          <TabsContent value="transactions" className="space-y-2 mt-3">
            {monthlyTransactions.length === 0 ? (
              <EmptyState icon={DollarSign} title="No transactions yet" description="Start tracking your income and expenses" actionLabel="Add transaction" onAction={openNew} />
            ) : monthlyTransactions.sort((a, b) => b.date.localeCompare(a.date)).map(t => <SwipeableTransaction key={t.id} transaction={t} onDelete={() => setDeleteId(t.id)} onEdit={() => openEdit(t)} />)}
          </TabsContent>
          <TabsContent value="breakdown" className="mt-3 space-y-4">
            ...
            <div className="space-y-3">
              {BUDGET_CATEGORIES.map(cat => {
                ...
                return <div key={cat} className="space-y-1"><div className="flex items-center justify-between text-sm"><span className="capitalize font-medium">{cat}</span><span className="text-muted-foreground">${spent.toFixed(0)}{limit ? ` / $${limit}` : ''}</span></div>{limit && <Progress value={pct} className="h-2" />}</div>;
              })}
            </div>
          </TabsContent>
        </Tabs>

        <motion.button className="fixed bottom-24 right-5 z-40 h-14 w-14 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center" whileTap={{ scale: 0.9 }} onClick={openNew}><Plus className="h-6 w-6" /></motion.button>

        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetContent side="bottom" className="rounded-t-2xl max-h-[85vh] overflow-y-auto">
            <SheetHeader><SheetTitle>{editingTransaction ? 'Edit Transaction' : 'Add Transaction'}</SheetTitle><SheetDescription>{editingTransaction ? 'Update transaction details' : 'Log income or expense'}</SheetDescription></SheetHeader>
            <div className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-2"><Button variant={form.type === 'expense' ? 'default' : 'outline'} onClick={() => setForm({ ...form, type: 'expense' })}>Expense</Button><Button variant={form.type === 'income' ? 'default' : 'outline'} onClick={() => setForm({ ...form, type: 'income' })}>Income</Button></div>
              <div className="space-y-1.5"><Label>Description</Label><Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Coffee, Rent, Salary..." /></div>
              <div className="space-y-1.5"><Label>Amount</Label><Input type="number" inputMode="decimal" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0.00" /></div>
              <div className="space-y-1.5"><Label>Category</Label><Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v as BudgetCategory })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{BUDGET_CATEGORIES.map(c => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}</SelectContent></Select></div>
              <Button className="w-full" onClick={handleSave} disabled={!form.description.trim() || !form.amount}>{editingTransaction ? 'Save Changes' : 'Add Transaction'}</Button>
            </div>
          </SheetContent>
        </Sheet>

        <ConfirmDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)} title="Delete transaction?" description="This transaction will be permanently removed." onConfirm={() => { if (deleteId) deleteTransaction(deleteId); setDeleteId(null); }} />
      </div>
    </PullToRefresh>
  );
};
