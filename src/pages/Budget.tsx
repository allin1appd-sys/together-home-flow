...
const categoryColors: Record<BudgetCategory, string> = {
  ...
  utilities: 'hsl(190, 70%, 45%)',
  entertainment: 'hsl(340, 70%, 55%)',
  health: 'hsl(150, 60%, 45%)',
  shopping: 'hsl(45, 80%, 50%)',
  education: 'hsl(260, 50%, 55%)',
  savings: 'hsl(120, 50%, 45%)',
  other: 'hsl(0, 0%, 55%)',
};
...
const SwipeableTransaction = ({ transaction, onDelete }: SwipeableTransactionProps) => {
          ...
          <CardContent className="p-3 flex items-center justify-between">
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <Badge variant="outline" className="text-xs capitalize">{transaction.category}</Badge>
                <span className="text-xs text-muted-foreground">{format(parseISO(transaction.date), 'MMM d')}</span>
              </div>
            </div>
...
const Budget = () => {
  ...
  const handleSave = () => {
    ...
    setForm({ description: '', amount: '', category: 'food', type: 'expense' });
    setSheetOpen(false);
  };
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card>
          <CardContent className="p-3 text-center">
            ...
            <p className="font-bold text-sm text-green-600 dark:text-green-400">${totalIncome.toFixed(0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3 text-center">
            <TrendingDown className="h-4 w-4 text-red-600 dark:text-red-400 mx-auto mb-1" />
            <p className="text-xs text-muted-foreground">Expenses</p>
            <p className="font-bold text-sm text-red-600 dark:text-red-400">${totalExpenses.toFixed(0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3 text-center">
            ...
            <p className={`font-bold text-sm ${balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>${balance.toFixed(0)}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="transactions" className="space-y-2 mt-3">
          {monthlyTransactions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-muted-foreground mt-1">Tap + to log your first one</p>
            </div>
          ) : (
            monthlyTransactions
          ...
          {categoryData.length > 0 && (
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={categoryData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} innerRadius={40} paddingAngle={2}>
                    {categoryData.map((entry) => (
                      <Cell key={entry.name} fill={categoryColors[entry.name as BudgetCategory]} />
                    ))}
                  </Pie>
              ...
              return (
                <div key={cat} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">${spent.toFixed(0)}{limit ? ` / $${limit}` : ''}</span>
                  </div>
                  {limit && <Progress value={pct} className="h-2" />}
                </div>
              );
            })}
          </div>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl max-h-[85vh] overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>Log income or expense</SheetDescription>
          </SheetHeader>
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-2">
              <Button variant={form.type === 'expense' ? 'default' : 'outline'} onClick={() => setForm({ ...form, type: 'expense' })}>Expense</Button>
              <Button variant={form.type === 'income' ? 'default' : 'outline'} onClick={() => setForm({ ...form, type: 'income' })}>Income</Button>
            </div>
            <div className="space-y-1.5">
              <Label>Description</Label>
              <Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Coffee, Rent, Salary..." />
            </div>
            <div className="space-y-1.5">
              <Label>Amount</Label>
