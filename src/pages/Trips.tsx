...
const SwipeableTripCard = ({ trip, onTap, onDelete }: SwipeableTripCardProps) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl">
      <motion.div className="absolute inset-0 bg-destructive flex items-center justify-end pr-6 rounded-xl" style={{ opacity: trashOpacity }}>
...
const Trips = () => {
  ...
  const openEdit = (trip: Trip) => {
    ...
    setForm({
      ...
      description: trip.description || '', category: trip.category, status: trip.status,
      itinerary: trip.itinerary || [], packingList: trip.packingList || [],
    });
    setSheetOpen(true);
  };
  ...
  const addActivity = () => {
    ...
    setForm({
      ...form,
      itinerary: [...form.itinerary, { id: `act-${Date.now()}`, day: Math.min(form.itinerary.length + 1, tripDays), time: newActivity.time || '09:00', description: newActivity.description.trim() }],
    });
    setNewActivity({ time: '', description: '' });
  };

  const removeActivity = (id: string) => {
    setForm({ ...form, itinerary: form.itinerary.filter((a) => a.id !== id) });
  };
  ...
  const addPacking = () => {
    ...
    setForm({
      ...form,
      packingList: [...form.packingList, { id: `pack-${Date.now()}`, name: newPackingItem.trim(), isPacked: false }],
    });
    setNewPackingItem('');
  };
  ...
  const togglePacking = (id: string) => {
    setForm({
      ...form,
      packingList: form.packingList.map((p) => p.id === id ? { ...p, isPacked: !p.isPacked } : p),
    });
  };

  const removePacking = (id: string) => {
    setForm({ ...form, packingList: form.packingList.filter((p) => p.id !== id) });
  };
          ...
          <Tabs defaultValue="details" className="mt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}>
                    ...
                    <SelectContent>
                      {TRIP_CATEGORIES.map((c) => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
            ...
            <TabsContent value="itinerary" className="space-y-3 mt-3">
              <div className="flex gap-2">
                <Input className="w-20" type="time" value={newActivity.time} onChange={(e) => setNewActivity({ ...newActivity, time: e.target.value })} />
                <Input className="flex-1" value={newActivity.description} onChange={(e) => setNewActivity({ ...newActivity, description: e.target.value })} placeholder="Activity..." onKeyDown={(e) => e.key === 'Enter' && addActivity()} />
                <Button size="icon" variant="outline" onClick={addActivity}><Plus className="h-4 w-4" /></Button>
              </div>
              {form.itinerary.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-6">No activities yet — add your first one above</p>
              ) : (
                form.itinerary.map((act) => (
                  <div key={act.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted">
                    <Clock className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                    <span className="text-xs font-medium text-muted-foreground w-12">{act.time}</span>
                    <span className="text-sm flex-1 truncate">{act.description}</span>
                    <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => removeActivity(act.id)}>
            ...
            <TabsContent value="packing" className="space-y-3 mt-3">
              <div className="flex gap-2">
                <Input className="flex-1" value={newPackingItem} onChange={(e) => setNewPackingItem(e.target.value)} placeholder="Item to pack..." onKeyDown={(e) => e.key === 'Enter' && addPacking()} />
                <Button size="icon" variant="outline" onClick={addPacking}><Plus className="h-4 w-4" /></Button>
              </div>
              {form.packingList.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-6">No items yet — start your packing list above</p>
              ) : (
                form.packingList.map((item) => (
                  <div key={item.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted">
                    <Checkbox checked={item.isPacked} onCheckedChange={() => togglePacking(item.id)} />
                    <span className={`text-sm flex-1 ${item.isPacked ? 'line-through text-muted-foreground' : ''}`}>{item.name}</span>
                    <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => removePacking(item.id)}>
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  </div>
                ))
              ...
              {form.packingList.length > 0 && (
                <p className="text-xs text-muted-foreground text-center">
                  {form.packingList.filter((p) => p.isPacked).length}/{form.packingList.length} packed
                </p>
              )}
            </TabsContent>
          </Tabs>
