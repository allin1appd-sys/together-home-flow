...
const Dashboard = () => {
  ...
  const todayTasks = tasks.filter(t => t.dueDate === format(now, 'yyyy-MM-dd') && !t.isCompleted).sort((a, b) => {
  ...
  return (
    <div className="px-4 pt-6 space-y-5">
      <motion.div {...settle}><h1 className="text-2xl font-bold">Good {now.getHours() < 12 ? 'morning' : now.getHours() < 18 ? 'afternoon' : 'evening'}, {userName}</h1><p className="text-muted-foreground text-sm mt-0.5">{format(now, 'EEEE, MMMM d')}</p></motion.div>

      {expiringGroceries.length > 0 && <motion.div {...settle} transition={{ delay: 0.05 }}><button onClick={() => navigate('/groceries')} className="w-full"><Card className="border-warning/30 bg-warning/5"><CardContent className="flex items-center gap-3 p-4"><AlertTriangle className="h-5 w-5 text-warning shrink-0" /><div className="text-left"><p className="text-sm font-semibold">{expiringGroceries.length} item{expiringGroceries.length > 1 ? 's' : ''} expiring soon</p><p className="text-xs text-muted-foreground">{expiringGroceries.map(g => g.name).join(', ')}</p></div></CardContent></Card></button></motion.div>}

      {nextTrip && tripCountdown !== null && <motion.div {...settle} transition={{ delay: 0.1 }}><button onClick={() => navigate('/trips')} className="w-full"><Card className="border-primary/20 bg-accent/50"><CardContent className="flex items-center gap-3 p-4"><Plane className="h-5 w-5 text-primary shrink-0" /><div className="text-left"><p className="text-sm font-semibold">{nextTrip.title}</p><p className="text-xs text-muted-foreground">{nextTrip.destination} · {tripCountdown <= 0 ? 'Today!' : `in ${tripCountdown} day${tripCountdown > 1 ? 's' : ''}`}</p></div></CardContent></Card></button></motion.div>}

      <motion.div {...settle} transition={{ delay: 0.15 }}><Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><UtensilsCrossed className="h-4 w-4 text-primary" />Today's Meals</CardTitle></CardHeader><CardContent className="pt-0">{todayMeals.length === 0 ? <p className="text-sm text-muted-foreground">No meals planned — <button onClick={() => navigate('/meals')} className="text-primary underline">plan your day</button></p> : <div className="space-y-2">{todayMeals.map(m => <div key={m.id} className="flex items-center justify-between"><span className="text-xs font-medium text-muted-foreground uppercase w-16">{m.mealType}</span><span className="text-sm flex-1">{m.customMealName}</span></div>)}</div>}</CardContent></Card></motion.div>

      <motion.div {...settle} transition={{ delay: 0.2 }}><Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><CheckSquare className="h-4 w-4 text-primary" />Tasks Due Today{todayTasks.length > 0 && <Badge variant="secondary" className="ml-auto text-xs">{todayTasks.length}</Badge>}</CardTitle></CardHeader><CardContent className="pt-0">{todayTasks.length === 0 ? <p className="text-sm text-muted-foreground">All clear for today! ✨</p> : <div className="space-y-2">{todayTasks.slice(0, 5).map(task => <div key={task.id} className="flex items-center gap-3"><Badge className={cn('text-[10px] px-1.5 py-0', priorityColor[task.priority])}>{task.priority}</Badge><span className="text-sm flex-1 truncate">{task.title}</span></div>)}{todayTasks.length > 5 && <button onClick={() => navigate('/tasks')} className="text-xs text-primary">+{todayTasks.length - 5} more</button>}</div>}</CardContent></Card></motion.div>

      {upcomingReminders.length > 0 && <motion.div {...settle} transition={{ delay: 0.25 }}><Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><Bell className="h-4 w-4 text-primary" />Upcoming Reminders</CardTitle></CardHeader><CardContent className="pt-0 space-y-2">{upcomingReminders.map(r => <div key={r.id} className="flex items-center gap-3"><div className="h-2 w-2 rounded-full bg-warning shrink-0" /><span className="text-sm flex-1">{r.title}</span><span className="text-xs text-muted-foreground">{isToday(parseISO(r.dueDate)) ? 'Today' : isTomorrow(parseISO(r.dueDate)) ? 'Tomorrow' : format(parseISO(r.dueDate), 'MMM d')}</span></div>)}</CardContent></Card></motion.div>}

      <motion.div {...settle} transition={{ delay: 0.3 }}><button onClick={() => navigate('/budget')} className="w-full"><Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><DollarSign className="h-4 w-4 text-primary" />This Month's Budget</CardTitle></CardHeader><CardContent className="pt-0"><div className="flex items-center justify-between"><div><p className="text-xs text-muted-foreground">Spent</p><p className="text-lg font-bold flex items-center gap-1"><TrendingDown className="h-4 w-4 text-destructive" />${monthExpenses.toFixed(0)}</p></div><div className="text-right"><p className="text-xs text-muted-foreground">Balance</p><p className={cn('text-lg font-bold', monthBalance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-destructive')}>${monthBalance.toFixed(0)}</p></div></div></CardContent></Card></button></motion.div>
